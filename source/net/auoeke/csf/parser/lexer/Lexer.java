...
import net.auoeke.csf.parser.Context;
...
import net.auoeke.csf.parser.lexer.lexeme.CommaLexeme;
import net.auoeke.csf.parser.lexer.lexeme.CommentLexeme;
import net.auoeke.csf.parser.lexer.lexeme.DelimiterLexeme;
import net.auoeke.csf.parser.lexer.lexeme.Lexeme;
import net.auoeke.csf.parser.lexer.lexeme.MappingLexeme;
import net.auoeke.csf.parser.lexer.lexeme.NewlineLexeme;
import net.auoeke.csf.parser.lexer.lexeme.StringLexeme;
import net.auoeke.csf.parser.lexer.lexeme.Token;
import net.auoeke.csf.parser.lexer.lexeme.WhitespaceLexeme;
...
public class Lexer {
    ...
    public Lexer(String source, boolean retainSource, boolean throwExceptions, Csf.Option... options) {
        this.source = source;

        var optionSet = new HashSet<>(Arrays.asList(options));
        this.retainComments = optionSet.contains(Csf.Option.RETAIN_COMMENTS);
    ...
    public Lexer(String source, Csf.Option... options) {
    ...
    private char next() {
        this.current = this.source.charAt(this.nextIndex++);
            ...
            this.add(new StringLexeme(this.savedLine, this.savedColumn, String.valueOf(delimiter).repeat(length), buildString(builder -> {
                while (this.advance()) {
                    if (this.current == delimiter) {
                        ...
                        builder.append(this.source, this.previousIndex(), this.nextIndex += count);
                    } else {
                        builder.append(this.current);
                    }
                }
    ...
    private void rawString(char character) {
        ...
        var string = buildString(builder -> {
            ...
            while (this.advance()) {
                if (this.shouldTerminateExpression()) {
                    if (whitespace != -1) {
                        additionalLexemes.add(new WhitespaceLexeme(this.savedLine, this.savedColumn, this.source.substring(whitespace, this.previousIndex())));
                    }
